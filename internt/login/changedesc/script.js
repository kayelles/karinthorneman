...
function getDescFromPath(path) {
	for (i = 0; i < imageData["paths"].length; i++) {
		if ("../../" + imageData["paths"][i] == path) {
			return imageData["descs"][i];
		}
	}
}

function handleclick(caller, e) {
    var image_path = caller.attr("src");
    var olddesctag = getDescFromPath(image_path);
    var olddesc = olddesctag.substring(3, olddesctag.length - 4);
    var description = prompt("Ange en beskrivning: ", olddesc);	
    if (description != null) {
        image_id = getIDFromPath(image_path);
        $.post("changedesc.php", {path:image_path, id:image_id, desc:description}, function(data) {
